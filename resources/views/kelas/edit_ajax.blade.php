...
@else
<form action="{{ url('/kelas/' . $kelas->kelas_id . '/update_ajax') }}" method="POST" id="form-edit">
    @csrf
    @method('PUT')
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="font-weight: bold;">Edit Data Kelas</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="prodi_id" name="prodi_id" required>
                            ...
                            @foreach($prodiList as $p)
                                <option value="{{ $p->prodi_id }}" {{ $p->prodi_id == $kelas->prodi_id ? 'selected' : '' }}>{{ $p->prodi_nama }}</option>
                            @endforeach
                        </select>
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: response.message || 'Gagal mengupdate data kelas.',
                        });
                    }
                },
                error: function(xhr) {
