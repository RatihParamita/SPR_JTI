...
@else
<form action="{{ url('/prodi/' . $prodi->prodi_id . '/delete_ajax') }}" method="POST" id="form-confirm">
    @csrf
    @method('DELETE')
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="font-weight: bold;">Hapus Data Program Studi</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <span class="text-secondary">{{ $prodi->prodi_nama }}</span>
                    </div>
                </div>
                <div class="col-md-6 mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <span class="text-secondary">{{ $prodi->prodi_kode }}</span>
                    </div>
                </div>
            </div>
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: response.message || 'Gagal menghapus data program studi.',
                        });
                    }
                },
                error: function(xhr) {
