...
@else
<form action="{{ url('/dosen/' . $dosen->dosen_id . '/delete_ajax') }}" method="POST" id="form-confirm">
    @csrf
    @method('DELETE')
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="font-weight: bold;">Hapus Data Dosen</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
        <div class="modal-body">
            ...
            <div class="d-flex align-items-center mb-4">
                ...
                <div>
                    <h3 class="mb-0 font-weight-bold">{{ $dosen->dosen_nama }}</h3>
                    <p class="text-muted mb-0">ID Dosen: {{ $dosen->dosen_id }}</p>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-md-6 mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <span class="text-secondary">{{ $dosen->user->username }}</span>
                    </div>
                </div>
                <div class="col-md-6 mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <span class="text-secondary">{{ $dosen->prodi ? $dosen->prodi->prodi_nama : '-' }}</span>
                    </div>
                </div>
                <div class="col-md-6 mb-4">
                    ...
                    <div class="d-flex align-items-center">
                        ...
                        <span class="text-secondary">{{ $dosen->dosen_noHp }}</span>
                    </div>
                </div>
            </div>
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: response.message || 'Gagal menghapus data dosen.',
                        });
                    }
                },
                error: function(xhr) {
