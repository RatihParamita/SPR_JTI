...
<form action="{{ url('/dosen/' . $dosen->dosen_id . '/update_ajax') }}" method="POST" id="form-edit">
    ...
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" style="font-weight: bold;">Edit Data Dosen</h5>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
        </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label>NIDN</label>
                        <input type="text" class="form-control" id="dosen_nidn" name="dosen_nidn" value="{{ $dosen->dosen_nidn }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="prodi_id" name="prodi_id" required>
                            ...
                            @foreach ($prodiList as $prodi)
                                <option value="{{ $prodi->prodi_id }}" {{ $prodi->prodi_id == $dosen->prodi_id ? 'selected' : '' }}>{{ $prodi->prodi_nama }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="username" name="username" value="{{ $dosen->user->username }}" required>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        <label>Nama</label>
                        <input type="text" class="form-control" id="dosen_nama" name="dosen_nama" value="{{ $dosen->dosen_nama }}" required>
                        <small id="error-dosen_nama" class="error-text form-text text-danger"></small>
                    </div>
                    <div class="form-group">
                        <label>No. HP</label>
                        <input type="text" class="form-control" id="dosen_noHp" name="dosen_noHp" value="{{ $dosen->dosen_noHp }}" required>
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Gagal!',
                            text: response.message || 'Gagal mengupdate data dosen.',
                        });
                    }
                },
                error: function(xhr) {
